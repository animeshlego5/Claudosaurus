...
### Fixed
- **Settings panel layout** — replaced hardcoded pixel heights with dynamic
  `scrollHeight` sizing so the panel fits its content instead of clipping.
  Uses `scrollMarginBottom` for scroll clearance past the sticky input area
  (no more dead space beneath the panel).
