/*
 * Claude-Rex — shared patch logic (used by cli.js, install.js, uninstall.js)
 * ------------------------------------------------------------------
...
const START = "/*__CLAUDE_REX_START__ (injected by claude-rex; do not edit) */";
const END = "/*__CLAUDE_REX_END__*/";
...
function stripBlock(src) {
  ...
  if (e === -1) return src.slice(0, s).replace(/\s+$/, "") + "\n";
  return (src.slice(0, s) + src.slice(e + END.length)).replace(/\s+$/, "") + "\n";
}
...
function install(opts) {
  ...
  if (patched && !DRY) {
    ...
    log("Re-run after the extension updates. Undo any time: claude-rex uninstall");
  }
  return 0;
}
